...
def connect_to(instances, user='', cmd='', batch='', one_only='', public=False):
    ...
    if user:
        cmd_usr = ' -l {} '.format(user)
    else:
        cmd_usr = ' '
    print("choices of {} instances".format(len(instances)))
    if len(instances) < 2 or batch and not one_only:
        if len(instances) < 2 and cmd == "logs":
            ...
            print("connecting to: {} - {} ".format(name, ip))
        ...
        if len(dest) == 1:
            ...
            for inst in instances:
                if int(inst['index']) == int(dest[0]):
                    ...
                    if cmd:
                        print("Running {} on {}".format(cmd, inst['name']))
...
def init_tmux(instances, title='loony', cmd='', user='', public=False):
    ...
    for inst in instances:
        ...
        if new_window:
            ...
            b.cmd('kill-pane')
            new_window = False
        p.send_keys("echo 'connecting to  {}'".format(inst['name']))
        ...
        if cmd == 'logs':
            ...
            p.send_keys("echo 'tailing the following logs: {}'".format(logs))
            p.send_keys("sudo tail -n 0 -fq  {} | grep -v 'network_id cookie not set'".format(logs))
        elif cmd == 'syslogs':
            logs = " ".join(systemlogs)
            p.send_keys("echo 'tailing the following logs: {}'".format(logs))
            p.send_keys("sudo tail -n 0 -fq  {} ".format(logs))
        elif cmd:
            p.send_keys(cmd)
        pindex += 1
